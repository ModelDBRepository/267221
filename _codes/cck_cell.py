...
from cell_models import CCKCell
...
# for icur in np.flip(np.arange(start=-0.300, stop=-0.020, step=0.025)):
# for icur in np.arange(start=start, stop=stop, step=step):
# for icur in [-0.3, -0.10, 0.100, 0.350]:
for icur in [0.1]:
    print(icur)
    
    # Create a CCK Cell instance
    cell = CCKCell(0)
    # =========================================================================
    # SAVE VECTORS
    # =========================================================================
...
if plot_all:
    if stim.amp > 0:
        ...
        plt.figure(figsize=(16, 10))
        Inj = np.flip(np.arange(start=-0.300, stop=-0.020, step=0.025)) * 1e3
        plt.plot(
