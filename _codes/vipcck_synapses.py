...
from cell_models import VIPCCKCell
...
for nrep in range(replications):
    ...
    for ntrial in range(trials):
        ...
        eps = np.random.rand()
        if eps < 21/25:
            r1 = np.random.poisson(lam=21, size=1).item()
        else:
            r1 = 0
...
# plt.xticks([])
# plt.title("LEC driven response on VIP+/CCK+ cells")
# plt.show()
...
with open(dirname + 'vipcck.pkl', 'wb') as handle:
